        ...
        with open(cfg_path, "r", encoding="utf-8", errors="replace") as f:
            for raw_line in f:
                ...
                if section == "[server]":
                    ...
                    if key == "enabled":
                        result["web_enabled"] = _parse_bool(value, False)
                    elif key in ("only", "web_only"):
                        result["web_only"] = _parse_bool(value, False)
                    elif key == "domain":
                        result["web_domain"] = value
...
def _users_status() -> dict:
    ...
    # `[web].only` makes the proxy mask direct MTProto instead of serving it, so the
    # ee links below stop connecting entirely — mirrors Config.Web.onlyActive(), which
    # also requires `enabled`.
    web_enabled = bool(cfg.get("web_enabled"))
    web_domain = str(cfg.get("web_domain") or "").strip().lower().rstrip(".")
    web_only = web_enabled and bool(cfg.get("web_only")) and bool(web_domain)
    if not web_enabled:
        web_domain = ""
    ...
    for name in sorted(users.keys()):
        ...
        # WEB links carry the same 16-byte secret as `dd…`: Telegram Desktop reports an
        # `ee` FakeTLS secret as Unsupported for a WEB proxy. Port 443 is implicit.
        web_tg_link = None
        web_tme_link = None
        if web_domain:
            web_tg_link = f"tg://webproxy?server={web_domain}&secret=dd{secret_raw}"
            web_tme_link = f"https://t.me/webproxy?server={web_domain}&secret=dd{secret_raw}"
        ...
        items.append(
            {
                ...
                "tg_link": tg_link,
                "tme_link": tme_link,
                "web_tg_link": web_tg_link,
                "web_tme_link": web_tme_link,
            }
        )
    ...
    result = {
        ...
        # Under WEB-only the public address is irrelevant — the link carries the relay
        # domain — so a host with no detectable IP still has links to show.
        "links_ready": bool(server) or bool(web_domain),
        "server": server,
        "port": port,
...
# Kept byte-identical to tunnel_wg.OWNED_MARKER — this file and the Zig CLI must agree
# on what "mtbuddy created it" means, or the dashboard deletes what `mtbuddy uninstall`
# now refuses to (issue #402).
TUNNEL_OWNED_MARKER = "# Generated by mtbuddy — tunnel pool member; removed on `mtbuddy uninstall`."


def _tunnel_config_is_owned(path: Path) -> bool:
    """True when mtbuddy wrote this config, i.e. it is ours to delete.

    mtbuddy shares AmneziaWG's own /etc/amnezia/amneziawg directory, so an operator's
    configs sit right next to ours and the path proves nothing. A config adopted in
    place (`setup tunnel --iface awg1 /that/very/file`) is never stamped, and neither is
    one written before the banner existed; both are left alone.
    """
    try:
        # The banner is prepended, so a prefix read is enough however large the config
        # is. Note the Zig side must NOT use a small `.limited(n)` read for this: there
        # `n` fails the whole read rather than truncating, so a big config would come
        # back as "not ours" and the two implementations would disagree.
        with path.open("r", errors="replace") as fh:
            return TUNNEL_OWNED_MARKER in fh.read(8192)
    except Exception:
        return False


def _tunnel_has_foreign_config(interface: str) -> bool:
    """True when some config behind `interface` exists and is not mtbuddy's.

    The absence of any config is deliberately not "foreign": a pool member always had
    one when it was set up, so a missing file means a broken leftover.
    """
    for path in _tunnel_config_paths(interface):
        # The awg0 symlink is mtbuddy's by construction; judge on the real configs.
        if path == Path("/etc/amnezia/awg0.conf") or not path.exists():
            continue
        if not _tunnel_config_is_owned(path):
            return True
    return False
...
def _delete_tunnel_config_files(interface: str) -> bool:
    """Remove the configs for `interface`, but only the ones mtbuddy wrote.

    Returns False when something was left behind because it carries no ownership
    banner. The pool entry still goes, so the proxy stops routing through the
    ...
    for path in _tunnel_config_paths(interface):
        if not path.exists():
            continue
        if path != Path("/etc/amnezia/awg0.conf") and not _tunnel_config_is_owned(path):
            kept = True
            continue
        try:
            path.unlink()
...
def _delete_tunnel_interface(interface: str) -> dict | None:
    ...
    # Only tear the interface down when no config behind it is somebody else's. An
    # adopted interface was up before mtbuddy touched it and is theirs to run; removing
    # it from the pool already stops the proxy routing through it.
    if not _tunnel_has_foreign_config(iface):
        _stop_tunnel_interface(iface)
    result["kept_config"] = not _delete_tunnel_config_files(iface)
...
async def api_qr(text: str = ""):
    ...
    """
    text = (text or "").strip()
    # `tg://webproxy` does not match the `tg://proxy` prefix, so WEB links needed
    # spelling out — without them a WEB-only deploy has a QR button that always 400s.
    allowed_prefixes = (
        "https://t.me/proxy",
        "tg://proxy",
        "https://t.me/webproxy",
    ...
    if not text or len(text) > 512 or not text.startswith(allowed_prefixes):
