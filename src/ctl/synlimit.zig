...
///
/// `set -e` is what actually delivers that "fails the unit" guarantee: with `set -u`
/// alone the trailing `exit 0` overwrote every earlier status, so a rejected rule still
/// left systemd reporting `active (exited)` and the operator believing the limiter was
/// enforcing. `[ "$MODE" = flush ] && exit 0` remains safe under errexit (a non-final
/// member of an AND list is exempt), and `-w` keeps a boot-time xtables-lock collision
/// from failing the unit spuriously now that a non-zero status is fatal.
pub fn renderScript(buf: []u8, p: ScriptParams) ![]const u8 {
    return std.fmt.bufPrint(buf,
        ...
        \\ACTION="{[target]s}"
        \\# Wait for the xtables lock rather than exiting 4 on a concurrent iptables run.
        \\W="-w 5"
        \\
...
test "renderScript bakes params and stays brace-free for std.fmt" {
    ...
    // errexit is what makes a rejected rule actually fail the oneshot; without it the
    // trailing `exit 0` masks every earlier status and the unit reports success.
