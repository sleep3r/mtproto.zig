...
pub fn execute(ui: *Tui, allocator: std.mem.Allocator, opts: TunnelOpts) !void {
    ...
    // Stamped last, after the three normalizers have finished rewriting the file, so
    // the banner survives. Only for configs mtbuddy actually wrote: an adopted one is
    // the operator's, and `uninstall` keys its deletions on this marker.
    if (adopted_in_place) {
        ui.warn("Adopted your existing config in place — mtbuddy normalized it but will NOT delete it on uninstall");
    } else {
        stampOwned(allocator, awg_config_path) catch {
            ui.warn("Could not mark the config as mtbuddy-owned; uninstall will leave it in place");
        };
    }
...
/// Banner mtbuddy stamps into every tunnel config it writes for itself, and the only
/// thing that makes a config safe to delete later.
///
/// mtbuddy shares AmneziaWG's own directory — /etc/amnezia/amneziawg/<iface>.conf is
/// exactly where an operator's configs live too — so the path says nothing about who
/// created a file. Before this marker existed, `mtbuddy uninstall` globbed the whole
/// directory and `rm -rf`'d /etc/amnezia, which took an operator's pre-existing awg1
/// (and its private keys) with it: issue #402.
///
/// Safe to prepend to a wg-quick/awg-quick config: `#` is a comment there, and all
/// three of our in-place normalizers (`stripAwgDnsLines`, `stripAwgEmptyAssignments`,
/// `ensureAwgTableOff`) skip comment lines.
pub const OWNED_MARKER = "# Generated by mtbuddy — tunnel pool member; removed on `mtbuddy uninstall`.";

/// True when this config carries the marker, i.e. mtbuddy wrote it and may delete it.
/// A config mtbuddy only *adopted* (`--iface awgN /path/to/that/very/file`) is never
/// stamped, and an unmarked config predating the marker is treated the same way: left
/// alone. Both are reported rather than removed.
pub fn configIsOwned(allocator: std.mem.Allocator, path: []const u8) bool {
    // The same 1 MiB ceiling the normalizers use, deliberately: `.limited(n)` fails the
    // whole read at n bytes, so a smaller "we only need the first line" cap would make
    // every config at or above it read as foreign — and a config mtbuddy created would
    // then survive `uninstall` with its private key on disk.
    const content = readFileAllocCwd(allocator, path, 1024 * 1024) catch return false;
    defer allocator.free(content);
    return std.mem.indexOf(u8, content, OWNED_MARKER) != null;
}

/// Prepend the ownership banner unless it is already there (re-running `setup tunnel`
/// on a config we own must not stack banners).
fn stampOwned(allocator: std.mem.Allocator, path: []const u8) !void {
    const content = try readFileAllocCwd(allocator, path, 1024 * 1024);
    defer allocator.free(content);
    if (std.mem.indexOf(u8, content, OWNED_MARKER) != null) return;
    const stamped = try std.fmt.allocPrint(allocator, "{s}\n{s}", .{ OWNED_MARKER, content });
    defer allocator.free(stamped);
    try sys.writeFileMode(path, stamped, 0o600);
}

/// True when `source` and `dest_path` name the same file on disk — the shape of
/// `mtbuddy setup tunnel --iface awg1 /etc/amnezia/amneziawg/awg1.conf`, where mtbuddy
/// normalizes an operator's own config in place rather than creating one. That config
/// stays the operator's, so it is never stamped and never deleted.
///
/// `realpath` on a destination that does not exist yet fails, which is exactly the
/// answer we want there: nothing to adopt, so this is a create.
pub fn sourceIsDestination(source: []const u8, dest_path: []const u8) bool {
    const trimmed = std.mem.trim(u8, source, &[_]u8{ ' ', '\t', '\r', '\n' });
    if (isAmneziaVpnLinkSource(trimmed)) return false;
    var src_buf: [std.Io.Dir.max_path_bytes]u8 = undefined;
    var dst_buf: [std.Io.Dir.max_path_bytes]u8 = undefined;
    const src_len = std.Io.Dir.cwd().realPathFile(io(), trimmed, &src_buf) catch return false;
    const dst_len = std.Io.Dir.cwd().realPathFile(io(), dest_path, &dst_buf) catch return false;
...
pub fn selectTunnelInterface(allocator: std.mem.Allocator, requested: []const u8) ![]u8 {
    ...
    // An unreadable config.toml used to short-circuit straight to "awg0", skipping the
    // free-name scan below — so on a host whose config had been lost, `setup tunnel`
    // with no --iface would overwrite an operator's own awg0. Treat it as an empty pool
    // instead and let the scan pick a name nothing is using.
    var doc_opt = toml.TomlDoc.load(allocator, INSTALL_DIR ++ "/config.toml") catch null;
    defer if (doc_opt) |*d| d.deinit();

    const pool = if (doc_opt) |*d| try loadTunnelPoolFromDoc(allocator, d) else &[_][]const u8{};
...
test "ownership marker: only a stamped config is mtbuddy's to delete" {
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    ...
    const body =
        ...
        \\
    ;
    try tmp.dir.writeFile(io(), .{ .sub_path = "mine.conf", .data = body });
    try tmp.dir.writeFile(io(), .{ .sub_path = "theirs.conf", .data = body });

    var mine_buf: [std.Io.Dir.max_path_bytes]u8 = undefined;
    const mine = try std.fmt.bufPrint(&mine_buf, ".zig-cache/tmp/{s}/mine.conf", .{tmp.sub_path});
    var theirs_buf: [std.Io.Dir.max_path_bytes]u8 = undefined;
    const theirs = try std.fmt.bufPrint(&theirs_buf, ".zig-cache/tmp/{s}/theirs.conf", .{tmp.sub_path});

    // An operator's own config — exactly the shape issue #402 lost — is never ours.
    try std.testing.expect(!configIsOwned(std.testing.allocator, mine));
    try std.testing.expect(!configIsOwned(std.testing.allocator, theirs));

    try stampOwned(std.testing.allocator, mine);
    try std.testing.expect(configIsOwned(std.testing.allocator, mine));
    try std.testing.expect(!configIsOwned(std.testing.allocator, theirs));

    // The banner is prepended, not substituted: the config itself must survive intact.
    const stamped = try std.Io.Dir.cwd().readFileAlloc(io(), mine, std.testing.allocator, .limited(4096));
    defer std.testing.allocator.free(stamped);
    try std.testing.expect(std.mem.startsWith(u8, stamped, OWNED_MARKER));
    try std.testing.expect(std.mem.indexOf(u8, stamped, "PrivateKey = key") != null);
    try std.testing.expect(std.mem.indexOf(u8, stamped, "[Peer]") != null);

    // Re-running `setup tunnel` on a config we already own must not stack banners.
    try stampOwned(std.testing.allocator, mine);
    const restamped = try std.Io.Dir.cwd().readFileAlloc(io(), mine, std.testing.allocator, .limited(4096));
    defer std.testing.allocator.free(restamped);
    try std.testing.expectEqual(@as(usize, 1), std.mem.count(u8, restamped, OWNED_MARKER));

    // A missing file is not ours either — `configIsOwned` must never fail open.
    try std.testing.expect(!configIsOwned(std.testing.allocator, ".zig-cache/tmp/does-not-exist-9f3a.conf"));
}

test "ownership marker: a config larger than any read window is still recognised" {
    // `readFileAlloc(.limited(n))` fails the WHOLE read at n bytes, so a cap chosen to
    // cover "just the first line" would classify every config at or above it as foreign
    // — and `uninstall` would then leave a config mtbuddy wrote, private key included,
    // sitting on disk. A many-peer config crosses 8 KiB easily.
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();

    var body: std.ArrayList(u8) = .empty;
    defer body.deinit(std.testing.allocator);
    try body.appendSlice(std.testing.allocator, "[Interface]\nPrivateKey = key\n");
    var i: usize = 0;
    while (i < 200) : (i += 1) {
        var peer_buf: [96]u8 = undefined;
        const peer = try std.fmt.bufPrint(&peer_buf, "\n[Peer]\nPublicKey = peer{d}\nAllowedIPs = 10.{d}.0.0/16\n", .{ i, i % 250 });
        try body.appendSlice(std.testing.allocator, peer);
    }
    try std.testing.expect(body.items.len > 8 * 1024);
    try tmp.dir.writeFile(io(), .{ .sub_path = "big.conf", .data = body.items });

    var path_buf: [std.Io.Dir.max_path_bytes]u8 = undefined;
    const path = try std.fmt.bufPrint(&path_buf, ".zig-cache/tmp/{s}/big.conf", .{tmp.sub_path});

    try std.testing.expect(!configIsOwned(std.testing.allocator, path));
    try stampOwned(std.testing.allocator, path);
    try std.testing.expect(configIsOwned(std.testing.allocator, path));
}

test "the ownership banner survives every in-place normalizer" {
    // Ordering matters: `execute` stamps LAST, but a re-run normalizes an already
    // stamped config. All three normalizers skip comment lines, and this is the test
    // that says so.
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    ...
    var path_buf: [std.Io.Dir.max_path_bytes]u8 = undefined;
    const path = try std.fmt.bufPrint(&path_buf, ".zig-cache/tmp/{s}/awg0.conf", .{tmp.sub_path});

    try std.testing.expect(try stripAwgDnsLines(std.testing.allocator, path));
    try std.testing.expect(try stripAwgEmptyAssignments(std.testing.allocator, path));
    try std.testing.expect(try ensureAwgTableOff(std.testing.allocator, path));
    try std.testing.expect(configIsOwned(std.testing.allocator, path));
}

test "adopting a config in place is recognised as adoption, a copy is not" {
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    ...
    const body =
        ...
        \\
    ;
    try tmp.dir.writeFile(io(), .{ .sub_path = "awg1.conf", .data = body });
    try tmp.dir.writeFile(io(), .{ .sub_path = "source.conf", .data = body });

    var dest_buf: [std.Io.Dir.max_path_bytes]u8 = undefined;
    const dest = try std.fmt.bufPrint(&dest_buf, ".zig-cache/tmp/{s}/awg1.conf", .{tmp.sub_path});
    var src_buf: [std.Io.Dir.max_path_bytes]u8 = undefined;
    const src = try std.fmt.bufPrint(&src_buf, ".zig-cache/tmp/{s}/source.conf", .{tmp.sub_path});

    // `mtbuddy setup tunnel --iface awg1 <that same file>` — issue #402's invocation.
    try std.testing.expect(sourceIsDestination(dest, dest));
    // Whitespace around the argument must not change the answer.
    var padded_buf: [std.Io.Dir.max_path_bytes + 4]u8 = undefined;
    const padded = try std.fmt.bufPrint(&padded_buf, " {s}\n", .{dest});
    try std.testing.expect(sourceIsDestination(padded, dest));
    // A different file at the same destination is a copy: mtbuddy wrote it, mtbuddy owns it.
    try std.testing.expect(!sourceIsDestination(src, dest));
    // A vpn:// link is content, not a path — always a create.
    try std.testing.expect(!sourceIsDestination("vpn://abc", dest));
    // A destination that does not exist yet cannot be an adoption.
    var missing_buf: [std.Io.Dir.max_path_bytes]u8 = undefined;
    const missing = try std.fmt.bufPrint(&missing_buf, ".zig-cache/tmp/{s}/awg7.conf", .{tmp.sub_path});
    try std.testing.expect(!sourceIsDestination(src, missing));
}
