...
**Configs mtbuddy did not create are never deleted.** mtbuddy writes into
`/etc/amnezia/amneziawg/<iface>.conf` — AmneziaWG's own directory, where your configs live
too — so every config it writes carries a `# Generated by mtbuddy` banner, and only those
are removed by `mtbuddy uninstall` or `tunnel delete`. Point `--iface awg1` at an
interface's existing config and mtbuddy *adopts it in place*: it normalises the file (drops
`DNS`, adds `Table = off`) and adds the name to the pool, but leaves it, and the running
interface, alone when you uninstall. Anything left behind is listed by path at the end of
the uninstall so you can decide about it yourself.
...
| `[web] only` | `false` | Serve *only* WEB: direct MTProto is masked for everyone but the relay. Needs `mask = true`; ignored unless `enabled` |
...
**Bring your own certificate** if you already have one — acme.sh, a wildcard, a Cloudflare
Origin certificate with Cloudflare in front. `mtbuddy` then issues nothing, installs no
renewal hook, and points the vhost at your files; the paths are persisted in `[web]` so
`mtbuddy update` keeps them:
...
The certificate still has to be one the client's WebView will accept — that means a public
CA, or a private one only where whatever sits in front of you is what presents it to the
client. A self-signed certificate served straight to clients is refused by the WebView and
the bridge never loads.
...
- **WEB-only (`[web].only`)** takes that one step further: the proxy stops answering
  direct MTProto altogether. A FakeTLS or `dd` handshake from anyone but the relay is
  handed to the masking backend on the same code path a wrong secret takes, so a probe
  cannot tell the difference — see below.
...
Some networks blocklist an IP the moment a Telegram client connects to it directly — the
proxy works, and then the whole host stops answering on every TCP port for days. If that
is what you are seeing, stop offering the direct door:
...
masking backend, byte for byte the same way a wrong secret is, so the proxy is
indistinguishable from the website it fronts. `mtbuddy links` prints only the WEB links,
because they are the only ones that still connect.

What it costs:

- **Every `ee`/`dd` link you have already handed out stops working.** There is no partial
  rollout; turn it off with `sudo mtbuddy setup web --no-only`.
- **Desktop 7.1+ only** — phones cannot use a WEB link at all.
- **`[web].max_sessions` becomes your user ceiling**, since every client now arrives
  through the relay.
- It needs `[censorship].mask = true`. Without a cover site the refusal is a bare close,
  which is a *cleaner* signal to a prober than answering MTProto was — `mtbuddy` refuses
  to enable WEB-only without the local masking backend for exactly that reason.

`mtproto_web_only_masked_total` counts what the gate turned away.
