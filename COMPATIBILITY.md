...
- The WEB link format (`tg://webproxy?server=<host>&secret=<dd-secret>` and its `https://t.me/webproxy`
  ...
  `tls_domain` is.
- `[web].only` changes which links a deployment **emits**, never their format: with it set the proxy
  answers no direct MTProto, so `mtbuddy links` prints the WEB links alone. ⚠️ Turning it on
  invalidates every `ee`/`dd` link already distributed — the links are still well-formed, but nothing
  on the other end answers them any more. It is an operator decision, not a compatibility break.
...
- `mtbuddy uninstall` removes **only what mtbuddy installed**. For WireGuard/AmneziaWG tunnels that
  means the configs carrying its `# Generated by mtbuddy` banner; a config adopted in place via
  `setup tunnel --iface <name> <that same path>`, or one predating the banner, is left alone along
  with its running interface, and is reported by path at the end of the run. Removing more than that
  would be a bug, not a feature (it was: issue #402).
