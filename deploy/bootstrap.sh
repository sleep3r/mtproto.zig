#!/usr/bin/env bash
# bootstrap.sh — download and run buddy, the mtproto.zig installer
#
# Usage:
...
#
# After bootstrap, buddy lives at /usr/local/bin/buddy and can be called directly.

set -euo pipefail

REPO="sleep3r/mtproto.zig"
INSTALL_TO="/usr/local/bin/buddy"
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT
...
case "$ARCH" in
  x86_64)
    ...
    if grep -qE 'avx2|bmi1|bmi2' /proc/cpuinfo 2>/dev/null; then
      ARTIFACT="buddy-linux-x86_64_v3"
    else
      ARTIFACT="buddy-linux-x86_64"
    fi
    ;;
  aarch64) ARTIFACT="buddy-linux-aarch64" ;;
  *) fail "Unsupported architecture: $ARCH" ;;
esac

# ── resolve latest tag ────────────────────────────────────────────
step "Fetching latest buddy release"
...
curl -fsSL "$DOWNLOAD_URL" -o "$TMP/buddy.tar.gz" \
  || fail "Download failed: $DOWNLOAD_URL"

tar xzf "$TMP/buddy.tar.gz" -C "$TMP"
BUDDY_BIN="$(find "$TMP" -type f -name 'buddy' | head -1)"
[ -n "$BUDDY_BIN" ] || fail "buddy binary not found in archive"
