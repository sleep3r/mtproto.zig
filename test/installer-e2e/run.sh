...
# Issue #402: mtbuddy writes tunnel configs into AmneziaWG's OWN directory, so an
# operator's configs sit next to its own. Configs mtbuddy created carry a banner; a
# config adopted in place via `--iface <name> <that same path>` does not, and must
# survive both the adoption and the uninstall.
verify_tunnel_config_ownership() {
  local container="$1"

  run_script_in_container "$container" <<'CONTAINER_SCRIPT'
set -Eeuo pipefail

# An interface the operator set up themselves, long before mtbuddy, plus an unrelated
# file in the shared directory tree.
mkdir -p /etc/amnezia/amneziawg
cat >/etc/amnezia/amneziawg/awg9.conf <<'AWG_CONF'
[Interface]
PrivateKey = operator-private-key
Address = 10.200.0.2/32

[Peer]
PublicKey = operator-peer-key
Endpoint = 203.0.113.9:51820
AllowedIPs = 0.0.0.0/0
AWG_CONF
echo "operator data" >/etc/amnezia/operator-keep.txt

fail=0

# Exactly the invocation from issue #402: point --iface at the config's own path.
mtbuddy setup tunnel --iface awg9 /etc/amnezia/amneziawg/awg9.conf

if [ ! -f /etc/amnezia/amneziawg/awg9.conf ]; then
  echo "FAIL: adopting a config in place destroyed it"; fail=1
fi
if grep -qF "Generated by mtbuddy" /etc/amnezia/amneziawg/awg9.conf; then
  echo "FAIL: an adopted config was marked as mtbuddy-owned"; fail=1
fi
if ! grep -qF "operator-private-key" /etc/amnezia/amneziawg/awg9.conf; then
  echo "FAIL: adoption did not preserve the operator's config body"; fail=1
fi

# Configs mtbuddy created for itself (awg0/awg1, copied in from /tmp) must be marked.
for conf in /etc/amnezia/amneziawg/awg0.conf /etc/amnezia/amneziawg/awg1.conf; do
  if [ -f "$conf" ] && ! grep -qF "Generated by mtbuddy" "$conf"; then
    echo "FAIL: $conf was created by mtbuddy but carries no ownership banner"; fail=1
  fi
done

[ "$fail" -eq 0 ] || exit 1
echo "tunnel ownership: adopted config left unmarked, created configs marked"
CONTAINER_SCRIPT
}
...
# Issue #402: the tunnel teardown may only remove configs mtbuddy created. Everything
# else in /etc/amnezia — including the directory itself — stays exactly as it was.
for f in /etc/amnezia/amneziawg/awg9.conf /etc/amnezia/operator-keep.txt ; do
  if [ ! -e "$f" ]; then echo "FAIL: uninstall deleted $f, which mtbuddy never created"; fail=1; fi
done
for f in /etc/amnezia/amneziawg/awg0.conf /etc/amnezia/amneziawg/awg1.conf ; do
  if [ -e "$f" ]; then echo "FAIL: leftover mtbuddy-created tunnel config $f"; fail=1; fi
done
# And it must say which files it left behind, or the operator cannot act on them.
if ! printf '%s\n' "$out" | grep -qF "/etc/amnezia/amneziawg/awg9.conf"; then
  echo "FAIL: uninstall did not report the config it left in place"; fail=1
fi
...
run_case() {
  ...
  echo "::group::Tunnel configs mtbuddy did not create are left alone ($base_image)"
  verify_tunnel_config_ownership "$container" 2>&1 | tee "$LOG_DIR/$safe.tunnel-ownership.log"
  echo "::endgroup::"
